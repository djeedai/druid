...
use crate::{
    Command, Data, Env, Event, Handled, InternalEvent, KeyEvent, NativeWindowDesc,
    NativeWindowHandle, PlatformError, Selector, Target, TimerToken, WidgetId, WindowDesc,
    WindowId,
};
...
struct Windows<T> {
    ...
    /// OS handles to platform objects parented to a native top-level window. The `Window<T>`
...
impl<T> Windows<T> {
    ...
    fn connect(&mut self, id: WindowId, handle: WindowHandle, ext_handle: ExtEventSink) {
        ...
        } else if !self.child_windows.contains_key(&id) {
    ...
    fn remove_child(&mut self, id: WindowId, parent_id: WindowId) -> Option<ChildWindow> {
        if let Some(parent_win) = self.windows.get_mut(&parent_id) {
            parent_win.children.remove(&id);
        }
...
impl<T: Data> Inner<T> {
    ...
    /// Called after a child window has been clsoed by the platform.
    ///
    /// Clean up the internal association with the parent and notify the delegate, if necessary.
    fn remove_child_window(&mut self, id: WindowId, parent_id: WindowId) {
        self.with_delegate(|del, data, env, ctx| del.window_removed(id, data, env, ctx));
        self.windows.remove_child(id, parent_id);
    }
...
impl<T: Data> AppState<T> {
    ...
    fn remove_child_window(&mut self, window_id: WindowId, parent_id: WindowId) {
        self.inner
            .borrow_mut()
            .remove_child_window(window_id, parent_id)
    }
...
impl<T: Data> WinHandler for DruidHandler<T> {
    ...
    fn destroy(&mut self) {
        if let Some(parent_id) = self.parent_window_id {
            log::debug!(
                "Removing child={:?} from window={:?}",
                self.window_id,
                parent_id
            );
            self.app_state
                .remove_child_window(self.window_id, parent_id);
        } else {
            log::debug!("Removing top-level window id={:?}", self.window_id);
