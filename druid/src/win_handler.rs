...
use crate::{
    ...
    NativeWindowHandle, PlatformError, Target, TimerToken, WindowDesc, WindowId,
};
...
impl<T> Windows<T> {
    ...
    fn add_child(&mut self, id: WindowId, handle: WindowHandle, parent_id: WindowId) {
        if let Some(parent_win) = self.windows.get_mut(&parent_id) {
            let child_win = ChildWindow { parent_id, handle };
            assert!(
            ...
            debug!("added child #{:?} to parent #{:?}", id, parent_id);
...
impl<T: Data> Inner<T> {
    ...
    fn set_native_layout(&mut self, id: WindowId, origin: Option<Point>, size: Option<Size>) {
        debug!(
            "process SET_NATIVE_WINDOW_LAYOUT(id={:?}, origin={:?}, size={:?})",
            id, origin, size
        );
        ...
        if let Some(win) = self.windows.child_windows.get_mut(&id) {
            ...
            debug!("Failed to find native window with id={:?}", id);
        }
    }
...
impl<T: Data> AppState<T> {
    ...
    // fn remove_child_window(&mut self, window_id: WindowId) {
    //     self.inner.borrow_mut().windows.remove_child(window_id);
    // }
