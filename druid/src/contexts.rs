...
use crate::{
    ...
    NativeWindowDesc, Notification, Point, Rect, SingleUse, Size, Target, TimerToken, WidgetId,
...
impl LifeCycleCtx<'_, '_> {
    ...
    pub fn request_native_window(&mut self, min_size: Size) {
        ...
        let desc = NativeWindowDesc::new(
            ...
            self.widget_state.id,
            size,
            // FIXME - no simple way to get absolute origin (== relative to top-level parent),
...
impl PaintCtx<'_, '_, '_> {
    ...
    pub fn with_child_ctx(&mut self, region: impl Into<Region>, f: impl FnOnce(&mut PaintCtx)) {
        let mut child_ctx = PaintCtx {
            ...
            region: region.into(),
            depth: self.depth + 1,
            native_origin: self.native_origin, // + self.widget_state.origin().to_vec2(),
        };
        // log::info!("PaintCtx.with_child_ctx[BEGIN]: self.native_origin={:?} self.widget.origin={:?} -> child_ctx.native_origin={:?}",
        //     self.native_origin,
        //     self.widget_state.origin(),
        //     child_ctx.native_origin
        // );
        f(&mut child_ctx);
        // log::info!(
        //     "PaintCtx.with_child_ctx[END]: child_ctx.native_origin={:?}",
        //     child_ctx.native_origin
        // );
